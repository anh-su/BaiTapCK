...
<a href="<?=BASE_URL?>/QuanlyDT" class="edit-link">
    <== Đăng Ký Đề Tài</a>
        <hr class="custom-hr">
        <form method="post" action="<?=BASE_URL?>/DSdetai/timkiem">
        ...
        <div class="modal fade" id="assignModal" tabindex="-1" role="dialog" aria-labelledby="assignModalLabel"
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <form id="assignForm" method="POST" action="<?=BASE_URL?>/DSdetai/phanCongGiangVien">
                        <div class="modal-body">
        ...
        <script>
            $(document).ready(function () {
                ...
                $('.btn-approve').on('click', function () {
                    ...
                    $.ajax({
                        url: '<?=BASE_URL?>/DSdetai/approveDetai',
                        type: 'POST',
                        data: { ID_DTSV: idDTSV },
                        dataType: 'json',
                        success: function (response) {
                ...
                $('#confirmRejectButton').on('click', function () {
                    // Gửi yêu cầu AJAX để cập nhật trạng thái
                    $.ajax({
                        url: '<?=BASE_URL?>/DSdetai/rejectDetai',
                        type: 'POST',
                ...
                $('#assignForm').on('submit', function (event) {
                    ...
                    $.ajax({
                        url: '<?=BASE_URL?>/DSdetai/phanCongGiangVien',
                        type: 'POST',
                        data: formData,
                        dataType: 'json',
                        success: function (response) {
